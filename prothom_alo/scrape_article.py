...
# init()
processed_urls = set()
with open('prothom_alo_news_articles.jsonl', 'r', encoding='utf-8') as file:
  for line in file:
...
def scrape_urls(default_date, start_idx = 0, workers=16):
  ...
  for i, element in enumerate(article_tags[start_idx:], start=start_idx):
    ...
    date_modified = datetime.strptime(date_modified, "%Y-%m-%dT%H:%M:%S.%fZ") if date_modified else None
    ...
    with open('prothom_alo_news_articles.jsonl', 'a', encoding='utf-8') as file:
...
def scrape_article(url, default_date, default_date_modified):
  ...
  author_ = soup.find('span', class_='contributor-name _8TSJC')
  author = author_.get_text().strip() if author_ else None
  # print(f"author: {author}")

  # category
  category_ = soup.find('a', class_='vXi2j')
  category = category_.get_text().strip() if category_ else None
  # print(f"category: {category}")

  # tag
  tag_list = soup.find_all('li', class_="_2ybCx") if soup.find('li', class_="_2ybCx") else None
  tag = []
  if tag_list:
  ...
  # content
  paragraphs_container = soup.find(class_='VzzDZ') if soup.find(class_='VzzDZ') else None
