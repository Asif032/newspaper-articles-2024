...
processed_urls = set()
with open('bd_pratidin_news_articles.jsonl', 'r', encoding='utf-8') as file:
  for line in file:
...
def scrape_urls(default_date, start_idx = 0, workers=20):
  ...
  for i, element in enumerate(article_tags[start_idx:], start=start_idx):
    ...
    with open('bd_pratidin_news_articles.jsonl', 'a', encoding='utf-8') as file:
      for article in articles:
        file.write(json.dumps(article, ensure_ascii=False) + '\n')
        processed_urls.add(article["url"])
    
    t1 = time.time()
    progress = math.ceil((i + 1) / total_articles * 50)
    completed_percentage = (i + 1) / total_articles * 100
    progress_bar = f"[{'=' * progress}{'.' * (50 - progress)}] {completed_percentage:.2f}%"
    eta_string = f"ETA: {get_eta(t0, t1, i + 1, total_articles)} | Elapsed: {get_elapsed_time(int(t1 - t0))}"
...
def scrape_article(url, default_date, default_date_modified):
  ...
  author = soup.find('span', class_='fs-6').get_text().strip() \
    if soup.find('span', class_='fs-6') else None

  # category
  first_slash = url.find('/', 8)
  second_slash = url.find('/', first_slash + 1)
  category = url[first_slash + 1:second_slash]

  # tag
  tag = []
  tag_container = soup.find('div', class_="tagArea shadow bg-body-tertiary rounded") \
    if soup.find('div', class_="tagArea shadow bg-body-tertiary rounded") else None
  
  if tag_container:
    li_container = tag_container.find('ul', class_='mt-2')
    if li_container: 
      tag_list = li_container.find_all('li') if li_container.find('li') else None
      if tag_list:
        for tag_ in tag_list:
          tag.append(tag_.get_text().strip())

  # title
  title = soup.find('h1', class_='card-title n_head').get_text().strip() \
    if soup.find('h1', class_='card-title n_head') else None
  
  # content
  paragraphs_container = soup.find('article') if soup.find('article') else None
...
if __name__ == "__main__":
  # scrape_article('https://www.bd-pratidin.com/national/2024/11/19/1051665', datetime.now(), datetime.now().date())
